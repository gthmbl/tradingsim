...
//fetch stock data
const YAHOO_API_URL = "https://yahoo-finance166.p.rapidapi.com/api/market/get-quote"; //Base URL for API
...
app.post("/api/register", async (req, res) => {
    ...
    try {
        ...
        const [existingUser] = await db.query("SELECT * FROM users WHERE email = ?", [email]); //ensures emails are unique
        ...
        const [result] = await db.query(
            ...
            [username, email, hashedPassword, 10000.00] //$10,000 initial balance
        );
...
app.get("/api/portfolio", authenticateToken, async (req, res) => {
    try {
        ...
        if (portfolio.length > 0) {
            ...
            const stockPrices = response.data.quoteResponse.result.reduce((acc, stock) => {
...
app.post("/api/buy-stock", authenticateToken, async (req, res) => {
    ...
    console.log("Received request to buy stocks:", { stockSymbol, quantity, price });
  ...
  app.post("/api/sell-stock", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (existingStock.length === 0) {
            return res.status(400).json({ error: "Stock not found in the portfolio" });
        }
